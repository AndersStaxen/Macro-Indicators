...
Original file is located at
    https://colab.research.google.com/drive/1mm11fenJhsQCKwCiJJ-UE2qhRRASH9BB
...
# Step 2: Initialize FRED with your API key
fred = Fred(api_key='95548467cd24d216055cd02862fe8ccf')

###--------------------------------------------------------------

# Step 3: Date Range for Filtering the Overall Dataset
start_date = '2016-01-01'
end_date = '2024-12-31'

###--------------------------------------------------------------

# Step 4: Output Excel Filename
output_file = 'Economic_Indicators.xlsx'
...
# Step 5: Indicators Dictionary with Frequency Mapping
...
indicators = {
    ...
    'Nominal GDP': {
        'code': 'GDP', 'frequency': 'quarterly'},
    'GNP': {
        'code': 'GNPCA', 'frequency': 'quarterly'},
    'Retail Sales': {
        'code': 'MRTSSM44000USS', 'frequency': 'monthly'},
    'Unemployment Rate': {
        'code': 'UNRATE', 'frequency': 'monthly'},
    'Labor Force Participation': {
        'code': 'CIVPART', 'frequency': 'monthly'},
    'Employment Level': {
        'code': 'PAYEMS', 'frequency': 'monthly'},

    # --- Inflation & Prices ---
    'PCE': {
        'code': 'PCEPI', 'frequency': 'monthly'},
    'Core PCE': {
        'code': 'PCEPILFE', 'frequency': 'monthly'},
    'CPI': {
        'code': 'CPIAUCSL', 'frequency': 'monthly'},
    'Core CPI': {
        'code': 'CPILFESL', 'frequency': 'monthly'},
    'M2 Money Stock': {
        'code': 'M2', 'frequency': 'weekly'},
    'M2 Money Stock Growth': {
        'code': 'M2REAL', 'frequency': 'monthly'},
    'Average Hourly Wages': {
        'code': 'CES0500000003', 'frequency': 'monthly'},

    # --- Production ---
    'Industrial Production': {
        'code': 'INDPRO', 'frequency': 'monthly'},
    'Capacity Utilization': {
        'code': 'TCU', 'frequency': 'monthly'},
    'Durable Goods Orders': {
        'code': 'DGORDER', 'frequency': 'monthly'},
    'Producer Price Index': {
        'code': 'PPIACO', 'frequency': 'monthly'},

    # --- Interest Rates & Credit ---
    'Federal Funds Rate': {
        'code': 'FEDFUNDS', 'frequency': 'monthly'},
    '2-Year Treasury Constant Maturity Rate': {
        'code': 'DGS2', 'frequency': 'daily'},
    '5-Year Treasury Constant Maturity Rate': {
        'code': 'DGS5', 'frequency': 'daily'},
    '10-Year Treasury Yield': {
        'code': 'DGS10', 'frequency': 'daily'},
    '30-Year Treasury Yield': {
        'code': 'DGS30', 'frequency': 'daily'},
    '30-Year Fixed Rate Mortgage Average': {
        'code': 'MORTGAGE30US', 'frequency': 'weekly'},
    'Aaa Corporate Bond Spread': {
        'code': 'AAAFF', 'frequency': 'daily'},
    'Baa Corporate Bond Spread': {
        'code': 'BAAFF', 'frequency': 'daily'},

    # --- Debt & Household Indicators ---
    'Total Public Debt': {
        'code': 'GFDEBTN', 'frequency': 'quarterly'},
    'Total Public Debt as % of GDP': {
        'code': 'GFDEGDQ188S', 'frequency': 'quarterly'},
    'Household Debt Service Payments as % of Disposable Personal Income': {
        'code': 'TDSP', 'frequency': 'quarterly'},
    'Household Debt-to-GDP': {
        'code': 'HDTGPDUSQ163N', 'frequency': 'quarterly'},
    'Federal Debt Held by Foreign Investors': {
        'code': 'FDHBFIN', 'frequency': 'quarterly'},
    'Delinquency Rate on Credit Card Loans': {
        'code': 'DRCCLACBS', 'frequency': 'quarterly'},
    'Delinquency Rate on Mortgages': {
        'code': 'DRSFRMACBS', 'frequency': 'quarterly'},

    # --- Financial Markets & Expectations ---
    'Consumer Sentiment': {
        'code': 'UMCSENT', 'frequency': 'monthly'},  # Add to freq map if not there
    'VIX Volatility Index': {
        'code': 'VIXCLS', 'frequency': 'daily'},
    'S&P 500': {
        'code': 'SP500', 'frequency': 'daily'},
    'Home Sales': {
        'code': 'EXHOSLUSM495S', 'frequency': 'monthly'},
}

###----------------------------------------------------------

"""# **Pulling & Cleaning Data**
---
"""

# Step 6: Splitting Variables
dataframes = []
...
# Step 7: Creates table using merged data on Date
merged_df = reduce(lambda left, right: pd.merge(left, right, on='Date', how='outer'), dataframes)
merged_df = merged_df[(merged_df['Date'] >= '2017-01-01') & (merged_df['Date'] <= '2024-12-31')]
merged_df = merged_df.sort_values(by='Date', ascending=False)

###--------------------------------------------------------------

# Step 7.1: Compute Percentage Change and Extend Indicators Dictionary

# Ensure Date is datetime and sorted
merged_df['Date'] = pd.to_datetime(merged_df['Date'])
merged_df = merged_df.sort_values('Date', ascending=False)

# Variables to calculate percentage changes for
variables = [
    'Retail Sales', 'PCE', 'Core PCE', 'CPI', 'Core CPI', 'Producer Price Index'
]

# Extract only monthly frequency data for % change calculation
monthly_pct_df = merged_df[['Date'] + variables].copy()
monthly_pct_df = monthly_pct_df.dropna(subset=variables, how='all')
monthly_pct_df['Date'] = pd.to_datetime(monthly_pct_df['Date'])

# Keep only the last date per month (to ensure consistent monthly sampling)
monthly_pct_df['YearMonth'] = monthly_pct_df['Date'].dt.to_period('M')
monthly_pct_df = monthly_pct_df.sort_values('Date')
monthly_pct_df = monthly_pct_df.drop_duplicates('YearMonth', keep='last')
monthly_pct_df['Date'] = monthly_pct_df['YearMonth'].dt.to_timestamp()  # Move to 1st of month
monthly_pct_df = monthly_pct_df.drop(columns='YearMonth')

# Calculate Percentage Changes
for var in variables:
        monthly_pct_df[f'{var} MoM %'] = monthly_pct_df[var].pct_change(1) * 100
        monthly_pct_df[f'{var} YoY %'] = monthly_pct_df[var].pct_change(12) * 100

# Merge these % change columns back into main merged_df (aligning on Date)
merged_df = pd.merge(merged_df, monthly_pct_df[['Date'] + [col for col in monthly_pct_df.columns if '%' in col]], on='Date', how='left')
...
# Extend indicators dict so new columns are included in correct frequency sheets
for var in variables:
    base_freq = indicators.get(var, {}).get('frequency')
    if base_freq:
            indicators[f'{var} MoM %'] = {'code': None, 'frequency': base_freq}
            indicators[f'{var} YoY %'] = {'code': None, 'frequency': base_freq}

###--------------------------------------------------------------

# Step 8: Separates master dataset into frequency-specific DataFrames (later used in Excel)
def create_frequency_df(merged_df, indicators, target_frequency):
    df = merged_df[['Date']].copy()
    for column in merged_df.columns:
        if column == 'Date':
            continue
        if indicators.get(column, {}).get('frequency') == target_frequency:
            df[column] = merged_df[column]
    df = df.dropna(how='all', subset=df.columns.difference(['Date']))
    return df

# Create frequency-specific DataFrames
daily_df = create_frequency_df(filtered_df, indicators, 'daily')
weekly_df = create_frequency_df(filtered_df, indicators, 'weekly')
monthly_df = create_frequency_df(filtered_df, indicators, 'monthly')
quarterly_df = create_frequency_df(filtered_df, indicators, 'quarterly')

# Remove rows with no data in each frequency-specific table
def drop_fully_empty_rows(df):
    return df.dropna(how='all', subset=df.columns.difference(['Date']))

weekly_df = drop_fully_empty_rows(weekly_df)
monthly_df = drop_fully_empty_rows(monthly_df)
quarterly_df = drop_fully_empty_rows(quarterly_df)
...
# STEP 9: Creates the separate sheets using the various DataFrames created above

# Normalize all monthly dates to the 1st of the month for consistency
...
with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    ...
    quarterly_df.to_excel(writer, sheet_name='Quarterly', index=False)

###--------------------------------------------------------------

# STEP 10: Excel formatting
...
# Set a fixed width for date column and apply nice formatting
